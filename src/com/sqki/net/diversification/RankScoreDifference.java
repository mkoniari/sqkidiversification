//TODO needs to check the algorithm for correct working.
...
//		}
		...
		for (int i = 0; i < diverse.getResultList().size(); i++) {
			double score=(1/diverse.getResultList().get(i).getRank())+(1/Double.parseDouble(diverse.getResultList().get(i).getRunID()));
			docIDmapRankScore.put(diverse.getResultList().get(i).getDocID(),score);
		}
		
		docIDmapDiffRankSort.clear();
		docIDmapDiffRankSort.clear();
		diverse.getResultList().clear();
	
		
		sorting(docIDmapRankScore);
